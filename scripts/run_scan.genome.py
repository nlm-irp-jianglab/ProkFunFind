...
in_xml_file="/home/jiangx6/data/database/UHGG/MGYG-HGUT/ipr_annot/"+basename+"/"+ basename+".xml"
...
def ScoreKO(Query,annotdict) -> float:
    try:
        domScore = [ annotdict[i] for i in Query.hit_keys if i in annotdict.keys()]
...
def AnnotateGene(gene:Gene,KOAnnotdict:OrderedDict[str,Any],exe:str = "InterProScan") -> None:
    if hasattr(gene,exe):
        ...
        ScoreList = [ ScoreKO(Query,KOAnnotdict[KO]) for KO in KOAnnotdict.keys()]
        index, value = max(enumerate(ScoreList), key=operator.itemgetter(1))
        annot = (list(KOAnnotdict.keys())[index],value) if value >0 else (None,0)
        #gene.annot = annot
        return(annot)
    else:
        annot = (None,0)
...
def DBSCANCluster(pos:List[int],weight:List[float]=None,eps:float=2, min_samples:float=0.9) -> List[int]:
    ...
    cluster = DBSCAN(eps=eps,min_samples=min_samples).fit(pos,sample_weight=weight)
    return(list(cluster.labels_))

for ct in g1.contigs.values():
    poslist,genelist,termlist,weightlist = CheckGeneQualifiers(ct=ct, scoredic=OrthScore_dict)
