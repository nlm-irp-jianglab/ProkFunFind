...
def export_gene_tab(genomeObj:Genome,detect_tool:str,cluster_tool:str,outprefix:str) -> None:
    ...
    for gene in genomeObj.genes.values():
        if hasattr(gene,detect_tool):
            cluster_annot = gene.contig+":Cl_"+str(getattr(gene,cluster_tool)) if hasattr(gene,cluster_tool) else "NA"
            function_annot = ";".join(gene.Functions) if hasattr(gene,"Functions") else "Unassigned Function"
            print(gene.id+"\t"+cluster_annot+"\t"+function_annot,file=f)
    f.close()

def export_gene_gff(genomeObj:Genome,detect_tool:str,cluster_tool:str,outprefix:str) -> None:
    """TODO: Docstring for export_gene_tab.
...
def report_all(system_dict:Dict[str, Any],status:int,genomeObj:Genome,outprefix:str,detect_tool:str,cluster_tool:str) -> None:
