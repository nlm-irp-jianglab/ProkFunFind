...
def find_gene_with_orthoID(orthoID,genomeObject,detect_tool):
    ...
    """
    gene_list = [gene.id for gene in genomeObject.genes.values() if hasattr(gene, detect_tool) and getattr(getattr(gene,detect_tool),"orthoID") == orthoID ]
    return(gene_list)


def check_gene_in_subsystem(system_dict,genomeObject,detect_tool,fun_string=[]):
    """
    Assign genes assigned with orthoID to the system_dict
    """
    fun_list = fun_string.copy()
    completeness = { "mandatory":0, "accessory":0, "mandatory_presence":0, "accessory_presence":0}
    if "analogs" in system_dict:
        _, analogs_status = check_gene_in_subsystem(system_dict["analogs"],genomeObject,detect_tool,fun_string = fun_list)

    if "name" in system_dict:
        fun_list.append(system_dict["name"])

        for sub_component in system_dict["components"]:
            _, status = check_gene_in_subsystem(sub_component,genomeObject,detect_tool,fun_string = fun_list)
            ...
            gene_list = find_gene_with_orthoID(system_dict["orthoID"],genomeObject,detect_tool)
            status = 0 
            if gene_list:
    ...
    status = 0 
    if completeness["mandatory"] == completeness["mandatory_presence"] and completeness["mandatory"] >0:
        status = 1 
    elif analogs_status:
        status = 1
...
def pipeline(system_file,genome_object,detect_tool):
    ...
    system_dict,status=check_gene_in_subsystem(system_dict,genomeObject=genome_object,detect_tool=detect_tool)
