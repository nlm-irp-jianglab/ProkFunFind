...
def run_emapper(config: dict, genome_in_path: str):
    ...
    cmd = [exec_path, '-i', genome_in_path+'.pff.faa', '-o',
           genome_in_path+'.emapper.annotations',
           '--temp_dir', config['main']['tmp_dir'], '--tax_scope',
           config['emapper'].get('tax_scope', 'bacteria'),
           '--override', '--cpu', config['main'].get('cpus', 1),
           '--data_dir', config['emapper']['datadir']]
...
def run_kofamscan(config: dict, genome_in_path):
    ...
    cmd = [exec_path, '--cpu', config['main'].get('cpus', 1),
           '--tmp-dir', config['main']['tmp_dir'], '--format', 'detail-tsv',
           '-p', config['kofamscan']['profile'], '-o',
           genome_in_path+'.kofam.tsv', genome_in_path+'.pff.faa']
    res = subprocess.run(cmd)
    if res.returncode != 0:
        raise RuntimeError("Failed to run: {}".format(" ".join(cmd)))
